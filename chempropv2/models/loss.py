...
class BoundedMSELoss(MSELoss):
    ...
    def calc(
        ...
        **kwargs,
        ...
        preds = torch.where(
            torch.logical_and(preds < targets, lt_targets), targets, preds
        )
        preds = torch.where(
            torch.logical_and(preds > targets, gt_targets), targets, preds
        )
...
class PinballLoss(LossFunction):
    ...
    def calc(self, preds: Tensor, targets: Tensor, **kwargs) -> Tensor:
        ...
        loss = torch.max((Q - 1.0) * errors, Q * errors)
        return loss

class SmoothPinballLoss(LossFunction):
    alias = "regression-smoothpinball"
    def __init__(self, 
                 lamb: float = 0.3,
                 margin: float = 1e-2,
                 **kwargs):
    ...
    def calc(self, preds: Tensor, targets: Tensor, **kwargs) -> Tensor:
        ...
        errors = targets - preds
        Q = quantiles.unsqueeze(0)
        #loss = Q * errors + self.alpha * torch.log(1 + torch.exp(-errors / self.alpha))
        loss = torch.max((Q - 1.0) * errors, Q * errors)
        #preds_diff = preds[:, 1:] - preds[:, :-1]
